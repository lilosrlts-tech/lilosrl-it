import Link from "next/link";
import { CookiePreferencesLink } from "@/components/layout/CookiePreferencesLink";
import { LiloLogo } from "@/components/layout/LiloLogo";
import {
  FOOTER_LEGAL_LINKS,
  FOOTER_LILO_LINKS,
  FOOTER_NOLEGGIO_LINKS,
  FOOTER_SERVIZI_LINKS,
  GOLD,
} from "@/lib/nav-config";
import { PhoneLink } from "@/components/shared/PhoneLink";
import { COMPANY, SITE_URL } from "@/lib/constants";
import type { ImpostazioniSito } from "@/types/impostazioni";

interface SiteFooterProps {
  impostazioni: ImpostazioniSito;
}

function FooterColumn({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h3 className="text-xs font-bold uppercase tracking-wider text-white">{title}</h3>
      <div className="mt-4">{children}</div>
    </div>
  );
}

function FooterLink({
  href,
  children,
  external,
}: {
  href: string;
  children: React.ReactNode;
  external?: boolean;
}) {
  const className = "text-sm text-slate-400 transition hover:text-white";
  if (external) {
    return (
      <a href={href} target="_blank" rel="noopener noreferrer" className={className}>
        {children}
      </a>
    );
  }
  return (
    <Link href={href} className={className}>
      {children}
    </Link>
  );
}

function SocialIcon({ children, label, href }: { children: React.ReactNode; label: string; href: string }) {
  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={label}
      className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-slate-700 text-slate-400 transition hover:border-[#D4AF37] hover:text-white"
    >
      {children}
    </a>
  );
}

function FacebookIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4 fill-current" aria-hidden="true">
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  );
}

function InstagramIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4 fill-current" aria-hidden="true">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
    </svg>
  );
}

function LinkedInIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4 fill-current" aria-hidden="true">
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 114.126 0 2.063 2.063 0 01-2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  );
}

export function SiteFooter({ impostazioni }: SiteFooterProps) {
  const year = new Date().getFullYear();
  const hasSocial =
    impostazioni.social_facebook ||
    impostazioni.social_facebook_autolavaggio ||
    impostazioni.social_instagram ||
    impostazioni.social_linkedin;

  return (
    <footer className="border-t border-slate-800 bg-slate-950 text-slate-300">
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="mb-10 flex flex-col gap-6 border-b border-slate-800 pb-10 lg:flex-row lg:items-start lg:justify-between">
          <LiloLogo variant="light" />
          <div className="text-sm text-slate-400 lg:text-right">
            <p className="font-medium text-slate-200">{COMPANY.legalName}</p>
            <p className="mt-1">P.IVA {COMPANY.vatNumber}</p>
            <p className="mt-1">{impostazioni.indirizzo_noleggio}</p>
            <p className="mt-2">
              <PhoneLink
                phone={impostazioni.telefono_noleggio}
                className="font-semibold transition hover:text-white"
                style={{ color: GOLD }}
              >
                {impostazioni.telefono_noleggio}
              </PhoneLink>
              {" · "}
              <a href={`mailto:${impostazioni.email_contatto}`} className="hover:text-white">
                {impostazioni.email_contatto}
              </a>
            </p>
          </div>
        </div>

        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          <FooterColumn title="Lilo S.r.l.">
            <ul className="space-y-2.5">
              {FOOTER_LILO_LINKS.map((item) => (
                <li key={item.href}>
                  <FooterLink href={item.href}>{item.label}</FooterLink>
                </li>
              ))}
            </ul>
          </FooterColumn>

          <FooterColumn title="Noleggio Trieste">
            <ul className="space-y-2.5">
              {FOOTER_NOLEGGIO_LINKS.map((item) => (
                <li key={item.href}>
                  <FooterLink href={item.href}>{item.label}</FooterLink>
                </li>
              ))}
              <li>
                <FooterLink href="/flotta">Tutta la flotta</FooterLink>
              </li>
            </ul>
          </FooterColumn>

          <FooterColumn title="Servizi">
            <ul className="space-y-2.5">
              {FOOTER_SERVIZI_LINKS.map((item) => {
                if ("requiresOfferta" in item && item.requiresOfferta && !impostazioni.offerta_attiva) {
                  return null;
                }
                return (
                  <li key={item.href}>
                    <FooterLink href={item.href} external={"external" in item && item.external}>
                      {item.label}
                    </FooterLink>
                  </li>
                );
              })}
              <li>
                <FooterLink href="/autolavaggio">Autolavaggio LILO Trieste</FooterLink>
              </li>
            </ul>
          </FooterColumn>

          <FooterColumn title="Note legali">
            <ul className="space-y-2.5">
              {FOOTER_LEGAL_LINKS.map((item) => (
                <li key={item.href}>
                  <FooterLink href={item.href}>{item.label}</FooterLink>
                </li>
              ))}
            </ul>
          </FooterColumn>
        </div>
      </div>

      <div className="border-t border-slate-800 bg-slate-950">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-5 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div id="gdpr-consent-slot" className="min-h-[2rem] shrink-0 text-xs text-slate-500">
            <CookiePreferencesLink />
          </div>

          <p className="text-center text-xs text-slate-500 lg:flex-1">
            © {year} {COMPANY.legalName} —{" "}
            <a href={SITE_URL} className="hover:text-slate-300">
              www.lilosrl.it
            </a>
          </p>

          {hasSocial ? (
            <div className="flex items-center justify-center gap-2 lg:justify-end">
              {impostazioni.social_facebook && (
                <SocialIcon href={impostazioni.social_facebook} label="Facebook noleggio LILO">
                  <FacebookIcon />
                </SocialIcon>
              )}
              {impostazioni.social_facebook_autolavaggio && (
                <SocialIcon
                  href={impostazioni.social_facebook_autolavaggio}
                  label="Facebook autolavaggio LILO"
                >
                  <FacebookIcon />
                </SocialIcon>
              )}
              {impostazioni.social_instagram && (
                <SocialIcon href={impostazioni.social_instagram} label="Instagram LILO">
                  <InstagramIcon />
                </SocialIcon>
              )}
              {impostazioni.social_linkedin && (
                <SocialIcon href={impostazioni.social_linkedin} label="LinkedIn LILO">
                  <LinkedInIcon />
                </SocialIcon>
              )}
            </div>
          ) : (
            <div className="hidden w-24 lg:block" aria-hidden="true" />
          )}
        </div>
      </div>
    </footer>
  );
}
