...
export function VeicoloCard({ veicolo }: VeicoloCardProps) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-brand-200 hover:shadow-lg">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        {badges.length > 0 && (
          <ul className="mt-2 flex min-h-[1.5rem] flex-wrap gap-1.5" aria-label="Caratteristiche principali">
            {badges.map((badge) => (
              <li
                key={badge}
        ...
        <div className="mt-3 border-t border-slate-100 pt-4">
          <div className="flex flex-col gap-3">
            ...
            <Link
              href={`/flotta/${veicolo.slug}`}
              className="w-full rounded-xl bg-brand-600 px-5 py-2.5 text-center text-sm font-semibold text-white transition hover:bg-brand-700"
            >
              Visualizza Dettagli
            </Link>
          </div>
